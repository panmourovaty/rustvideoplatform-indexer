...
/// Return the preview sprite path (relative to source_dir) for a video, or `None`.
///
/// The sprite is expected at `{source_dir}/{media_id}/preview-sprite.avif` and
/// is typically generated by the main platform for video hover-preview.
fn fetch_preview_sprite(source_dir: &str, media_id: &str) -> Option<String> {
    let path = format!("{source_dir}/{media_id}/preview-sprite.avif");
    if std::path::Path::new(&path).exists() {
        Some(format!("{media_id}/preview-sprite.avif"))
    } else {
        None
    }
}
